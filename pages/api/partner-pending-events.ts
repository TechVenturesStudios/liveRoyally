...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<PartnerPendingEventsResponse>
) {
  setCorsHeaders(res);
  ...
  try {
    ...
    console.error("partner-pending-events error:", error);
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to fetch pending events",
    });
  }
}
