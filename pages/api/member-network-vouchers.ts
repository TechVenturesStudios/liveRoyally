...
type MemberNetworkVouchersResponse =
  | {
      member: {
        id: string;
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<MemberNetworkVouchersResponse>
) {
  setCorsHeaders(res);
  ...
  try {
    ...
    if (!member.member_profiles.network_code) {
    ...
    const vouchers = await prisma.$queryRaw<Array<Record<string, unknown>>>`
      ...
      ORDER BY v.created_at DESC, v.expiration_date ASC NULLS LAST;
    `;
    ...
    console.error("member-network-vouchers error:", error);
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to fetch network vouchers",
    });
  }
}
